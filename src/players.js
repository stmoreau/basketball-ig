const players = [
    {id: 1, name: 'Stephane Moreau', title: 'Front Ent Developer', interest: 'High'},
    {id: 2, name: 'Erica Perris', title: 'Operational Risk Manager', interest: 'High'},
    {id: 3, name: 'Irene Castaneda', title: 'Off-page Search Manager, DailyFX', interest: 'High'},
    {id: 4, name: 'Jerome Johnson', title: 'UK New Business Manager', interest: 'High'},
    {id: 5, name: 'Nojan Baseri', title: 'Premium Account Executive', interest: 'High'},
    {id: 6, name: 'Will Hall-Smith', title: 'Senior Copywriter', interest: 'High'},
    {id: 7, name: 'Feyi Obisesan', title: 'Associate QA Tester', interest: 'High'},
    {id: 8, name: 'Thomas Rogers', title: 'Trading Services Trainee', interest: 'High'},
    {id: 9, name: 'Periklis Korkontzelos', title: 'Associate Data Scientist', interest: 'High'},
    {id: 10, name: 'Jan Riccius', title: 'Trading Services Trainee', interest: 'High'},
    {id: 11, name: 'Alex Walker', title: 'Business Analyst', interest: 'High'},
    {id: 12, name: 'Yohan Kariyawasan', title: 'Sales Trader', interest: 'High'},
    {id: 13, name: 'Eren Eraslan', title: 'MTF Analyst', interest: 'Medium'},
    {id: 14, name: 'Romans Markuns', title: 'Developer', interest: 'Medium'},
    {id: 15, name: 'Vivek Rughani', title: 'Product Owner - Marketing Technology', interest: 'Medium'},
    {id: 16, name: 'David Mitchell', title: 'Developer', interest: 'Low'}
];

export {players};
